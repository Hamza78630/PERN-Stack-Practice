        ...
        <div className="page-console">
            <div className="console-shell console-shell-narrow">
                ...
                <div className="message-log">
                    {messages.length === 0 ? (
                        <p className="activity-empty message-log-empty">No messages yet. Start the conversation.</p>
                    ) : (
                        messages.map((item) => (
                            <div className="message" key={item.id || item.clientMessageId}>
                                <div className="message-meta">
                                    <strong>{item.senderEmail || "User"}</strong>
                                    <span className="message-role">{item.senderRole || ""}</span>
                                </div>
                                <p>{item.text}</p>
                            </div>
                        ))
                ...
                <form className="message-form" onSubmit={handleSendMessage}>
                    <input
                        ...
                        maxLength={1000}
                    />
                    <button type="submit" className="btn-amber" disabled={!connected || !message.trim()}>
                        Send
                    </button>
                </form>
